...
fn main() {
    ...
    let operation = args.get(2).map(|arg| arg.to_string()).unwrap_or_else(|| "insert".to_string());
    let num_of_items = args.get(3).and_then(|arg| arg.parse::<i32>().ok()).unwrap_or(1_000_000);
